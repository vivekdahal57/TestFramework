package com.vh.mi.automation.impl.pages.analytics.providerManagement.physicianManager;

import com.vh.mi.automation.impl.comp.dataGrid.DefaultColumnIdExtractor;

import java.util.regex.Pattern;

public class PhysicianManagerColumnIdExtractor extends DefaultColumnIdExtractor {
    private static final Pattern PHYSICIANMANAGER_HEADER_ID_PATTERN = Pattern.compile("^.*column_(.*)header$");
    private static final Pattern PHYSICIANMANAGER_SORT_LINK_ID_PATTERN = Pattern.compile("^.*column_(.*)_sortCommandLink$");
    private static final Pattern DYNAMIC_COLUMN_ID_PATTERN = Pattern.compile("^d2Form:simpleGrid.*:(.*)header$");


    @Override
    protected Pattern getColumnHeaderIdPattern() {
        return PHYSICIANMANAGER_HEADER_ID_PATTERN ;
    }

    @Override
    protected Pattern getSortLinkIdPattern() {
        return PHYSICIANMANAGER_SORT_LINK_ID_PATTERN ;
    }

    @Override
    protected Pattern getDynamicColumnIdPattern() {
        return DYNAMIC_COLUMN_ID_PATTERN;
    }
}
